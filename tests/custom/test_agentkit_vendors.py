...
from agora_agent.agentkit.vendors import GenericAvatar, OpenAI, OpenAIRealtime, XaiGrok, XaiRealtime
...
def test_xai_realtime_deprecated_alias_emits_same_vendor():
    with warnings.catch_warnings(record=True) as caught:
        warnings.simplefilter("always", DeprecationWarning)
        config = XaiRealtime(api_key="xai-key").to_config()
    assert len(caught) == 1
...
def test_llm_greeting_configs_interruptable_serializes():
    config = OpenAI(
        api_key="openai-key",
        greeting_configs=LlmGreetingConfigs(mode="single_first", interruptable=False),
    ).to_config()
